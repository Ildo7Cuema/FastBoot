...
class AndroidDeviceManager {
  ...
  async captureScreenshot(deviceId) {
    return new Promise((resolve, reject) => {
      const tempPath = path.join(os.tmpdir(), `screenshot-${deviceId}-${Date.now()}.png`);
      const devicePath = '/sdcard/screenshot.png';
      
      // Capturar screenshot no dispositivo
      exec(`${this.adbPath} -s ${deviceId} shell screencap -p ${devicePath}`, (error) => {
        if (error) {
          reject(error);
          return;
        }
        ...
        exec(`${this.adbPath} -s ${deviceId} pull ${devicePath} ${tempPath}`, (error) => {
          if (error) {
            reject(error);
            return;
          }
          ...
          fs.readFile(tempPath, (err, data) => {
            if (err) {
              ...
              fs.unlink(tempPath, () => {});
              exec(`${this.adbPath} -s ${deviceId} shell rm ${devicePath}`, () => {});
              resolve(data);
            }
          });
        });
  ...
  async installAPK(deviceId, apkPath) {
    return new Promise((resolve, reject) => {
      this.logger.info(`Instalando APK ${apkPath} no dispositivo ${deviceId}...`);
      
      exec(`${this.adbPath} -s ${deviceId} install -r "${apkPath}"`, 
  ...
  async createBackup(deviceId, options = {}) {
    return new Promise((resolve, reject) => {
      ...
      const backupPath = path.join(backupDir, `backup-${deviceId}-${Date.now()}.ab`);
      ...
      exec(backupCmd, { timeout: 600000 }, // 10 minutos de timeout
        (error, stdout, stderr) => {
          if (error) {
  ...
  async restoreBackup(deviceId, backupPath) {
    return new Promise((resolve, reject) => {
      this.logger.info(`Restaurando backup ${backupPath} no dispositivo ${deviceId}...`);
      
      exec(`${this.adbPath} -s ${deviceId} restore "${backupPath}"`, 
  ...
  async uninstallPackage(deviceId, packageName) {
    return new Promise((resolve, reject) => {
      this.logger.info(`Desinstalando pacote ${packageName} do dispositivo ${deviceId}...`);
      
      exec(`${this.adbPath} -s ${deviceId} uninstall ${packageName}`, 
        (error, stdout, stderr) => {
          if (error) {
  ...
  async setDeveloperMode(deviceId, enabled) {
    return new Promise((resolve, reject) => {
      ...
      exec(`${this.adbPath} -s ${deviceId} shell settings put global development_settings_enabled ${value}`, 
        (error, stdout, stderr) => {
          if (error) {
            reject(error);
          } else {
