...
class Logger extends EventEmitter {
  constructor(options = {}) {
    ...
    this.logs = [];
    this.maxLogs =
      options.maxLogs || parseInt(process.env.LOG_MAX_FILES) || 1000;
    this.logLevels = {
      DEBUG: 0,
      INFO: 1,
      WARN: 2,
    ...
    try {
      ...
      if (fs.existsSync(currentLogFile)) {
        ...
        if (stats.size >= this.maxFileSize) {
          ...
          const rotatedFile = currentLogFile.replace(
            '.log',
            `-${timestamp}.log`
          );
  ...
  flushBuffer() {
    ...
    try {
      ...
      const logLines = this.writeBuffer
  ...
  getLogsByDevice(deviceId) {
    return this.logs.filter(
      log =>
        log.message.includes(`[${deviceId}]`) ||
        (log.data && log.data.deviceId === deviceId)
    );
  }
   ...
   */
  searchLogs(query) {
    const { level, startDate, endDate, message, context, limit = 100 } = query;
